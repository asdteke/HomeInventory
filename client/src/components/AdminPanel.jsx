...
const USER_FILTERS = ['all', 'active', 'banned', 'admin'];
...
function normalizeAdminLogs(raw) {
    const source = raw && typeof raw === 'object' ? raw : {};
    return {
...
function formatAdminDate(value, locale, options = {
    month: 'short',
    day: 'numeric',
    hour: '2-digit',
    minute: '2-digit'
...
function sanitizeAdminLogDetailText(details) {
...
function parseAdminLogDetails(details) {
    if (!details) {
        return null;
    }
...
function formatAdminLogDetails(item, t) {
...
function formatAdminLogType(item, t) {
...
function formatAdminLogAction(item, t) {
...
function OverviewMetricCard({ icon: Icon, label, value, description, tone = 'accent', chips = [] }) {
    const toneMap = {
        accent: {
...
function ActivityFeed({ items, locale, t, emptyTitle, emptyDescription }) {
    if (!items.length) {
        return (
            <EmptyState
                icon={Activity}
...
function UserActionButton({ icon: Icon, label, tone = 'default', onClick }) {
...
export default function AdminPanel() {
    ...
    const loadActiveSection = async (tabId, { forceRefresh = false } = {}) => {
        if (!forceRefresh) {

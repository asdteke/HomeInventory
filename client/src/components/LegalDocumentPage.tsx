...
import { ArrowLeft, Moon, Sun, LucideIcon } from 'lucide-react';
...
function splitBody(rawBody: string): SplitBodyResult {
...
function renderInlineEmphasis(text: string | null | undefined): React.ReactNode[] {
...
function createSectionId(title: string, index: number, usedIds: Map<string, number>): string {
...
function looksLikeDocumentHeading(block: string): boolean {
...
interface ParsedSection extends SplitBodyResult {
    title: string;
    id: string;
    body: string;
}
...
export interface SummaryBlockItem {
    ...
    description?: string;
    items: string[];
    shortcuts?: { label: string; title: string }[];
}

...
export default function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // If backend says requiresTwoFactor even on error (wrong code), stay in 2FA mode
    ...
    return (
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 px-4 py-5 sm:py-8 transition-colors duration-300">
            <div className="mx-auto flex min-h-[calc(100dvh-2.5rem)] w-full max-w-md flex-col justify-center">
                <div className="mb-5 flex items-center justify-center gap-2 sm:justify-end">
                    <div className="w-full max-w-[240px] sm:w-auto sm:max-w-none">
                        <LanguageSwitcher className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm" />
                    </div>
                    <button onClick={toggleTheme} className="shrink-0 p-3 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:scale-110 transition-all shadow-sm">
                ...
                <div className="text-center mb-8 animate-fade-in">
                    <Link to="/">
                        <BrandLogo variant="full" size="md" className="mx-auto w-auto mb-4 max-h-[62px] sm:max-h-[76px]" />
                    </Link>
                    ...
                    {!requires2FA ? (
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            <div className="text-center mb-4">
                                <div className="w-16 h-16 mx-auto mb-3 rounded-full bg-primary-100 dark:bg-primary-500/20 flex items-center justify-center">
                                    <ShieldCheck className="w-8 h-8 text-primary-600 dark:text-primary-400" />
                                </div>
                                <h3 className="text-lg font-semibold text-slate-900 dark:text-white">
                                    {t('auth.login.two_factor.title')}
                                </h3>
                                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                                    {useBackupCode 
                                        ? t('auth.login.two_factor.backup_subtitle')
                                        : t('auth.login.two_factor.subtitle')
                                    }
                                </p>
                            </div>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                                    {useBackupCode 
                                        ? t('auth.login.two_factor.backup_code_label')
                                        : t('auth.login.two_factor.code_label')
                                    }
                                </label>
                                <input
                                    type="text"
                            ...
                            <label className="flex items-center gap-3 cursor-pointer group">
                                ...
                                <span className="text-sm text-slate-600 dark:text-slate-400 group-hover:text-slate-800 dark:group-hover:text-slate-200 transition-colors">
                                    {t('auth.login.two_factor.remember_device')}
                                </span>
                            </label>

                            <button type="submit" disabled={loading} className="btn-primary w-full py-3 text-lg disabled:opacity-50">
                                {loading 
                                    ? <span className="flex items-center justify-center gap-2"><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>{t('auth.login.submitting')}</span> 
                                    : t('auth.login.two_factor.verify')
                                }
                            </button>
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                >
                                    ...
                                    {useBackupCode 
                                        ? t('auth.login.two_factor.use_totp')
                                        : t('auth.login.two_factor.use_backup')
                                    }
                                </button>
                            </div>
                        </form>
                        ...
                        <>
                            ...
                            <div className="mt-4 p-4 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700/50">
                                ...
                                <div className="text-xs text-amber-700 dark:text-amber-400 space-y-2">
                                    <p>{t('auth.login.terms_beta')}</p>
                                    <p>{t('auth.login.terms_disclaimer')}</p>
                                    <p>{t('auth.login.terms_backup')}</p>
                                    <p className="text-amber-600 dark:text-amber-300 font-medium">{t('auth.login.terms_accept')}</p>
                                </div>
                            </div>
                        </>
                    )}

...
export default function Register() {
    ...
    const pageClass = isDark
        ? (false ? 'bg-[var(--hi-bg-strong)] text-white' : 'bg-[#1a1f1c] text-white')
    ...
    const subtleTextClass = isDark
        ? (false ? 'text-[var(--hi-text-muted)]' : 'text-white/55')
        : 'text-[var(--hi-text-soft)]';
    const iconMutedClass = isDark
        ? (false ? 'text-[var(--hi-text-muted)]' : 'text-white/28')
        : 'text-[var(--hi-text-muted)]';
    const iconButtonClass = isDark
        ? (false ? 'text-[var(--hi-text-muted)] hover:text-white' : 'text-white/32 hover:text-white/68')
    ...
    const dividerClass = isDark
        ? (false ? 'border-[var(--hi-border)]' : 'border-white/8')
    ...
    const panelSurfaceClass = isDark
        ? (false ? 'border-[var(--hi-border)] bg-[rgba(24,31,44,0.72)]' : 'border-white/8 bg-white/[0.02]')
        : (false
    ...
    const inactiveModeClass = isDark
        ? (false ? 'text-[var(--hi-text-muted)] hover:text-white/88' : 'text-white/54 hover:text-white/78')
    ...
    return (
        <div className={`landing-page-shell relative min-h-screen overflow-hidden ${pageClass}`}>
            ...
            <div className={`absolute left-[-7rem] top-16 h-56 w-56 rounded-full blur-3xl ${isDark ? (false ? 'bg-[rgba(139,180,255,0.10)]' : 'bg-[rgba(205,176,136,0.10)]') : (false ? 'bg-[rgba(139,180,255,0.16)]' : 'bg-[rgba(205,176,136,0.16)]')}`} />
            <div className={`absolute bottom-8 right-[-5rem] h-64 w-64 rounded-full blur-3xl ${isDark ? (false ? 'bg-[rgba(103,227,242,0.12)]' : 'bg-[rgba(74,125,100,0.14)]') : (false ? 'bg-[rgba(18,158,154,0.10)]' : 'bg-[rgba(45,82,65,0.10)]')}`} />
            ...
            <div className="relative z-10 flex min-h-screen items-center justify-center px-5 py-8 sm:px-6 sm:py-12">
                <div className="w-full max-w-[31rem]">
                        ...
                        <div className="ml-auto flex shrink-0 items-center gap-2 sm:gap-3">
                            ...
                            <div className="w-[140px] sm:w-[164px]">
                                <LanguageSwitcher
                                    showTooltip={false}
                                    showCodeBadge={false}
                                    className={`!h-10 !rounded-full !px-3 !py-0 sm:!h-11 sm:!px-4 ${isDark ? (false ? '!border-[var(--hi-border)] !bg-[var(--hi-panel)] !text-white hover:!bg-[var(--hi-panel-muted)]' : '!border-white/10 !bg-white/4 !text-white/88 hover:!bg-white/8') : '!border-[var(--hi-border)] !bg-[var(--hi-panel)] !text-[var(--hi-text)] hover:!bg-[var(--hi-panel-strong)]'}`}
                                />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className={`relative overflow-hidden rounded-[var(--hi-radius-md)] ${cardClass}`}>
                        <div className={`landing-panel-glow absolute inset-0 ${false && isDark ? 'opacity-30' : 'opacity-50'}`} />

                        <div className={`relative p-6 sm:p-8 ${false && isDark ? 'bg-[linear-gradient(180deg,rgba(17,24,35,0.24),rgba(17,24,35,0.08))]' : ''}`}>
                            ...
                            <div className="mt-8 sm:mt-9">
                                ...
                                <form onSubmit={handleSubmit} noValidate className="space-y-5">
                                    ...
                                    {mode === 'join' && (
                                        <div>
                                            ...
                                            <div className="group relative">
                                                <Key className={`absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 transition ${false ? 'group-focus-within:text-[var(--hi-accent)]' : 'group-focus-within:text-[#6f9978]'} ${iconMutedClass}`} />
                                                <input
                                                    type="text"
                                                    name="house_key"
                                    ...
                                    <div>
                                        ...
                                        <div className="group relative">
                                            <User className={`absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 transition ${false ? 'group-focus-within:text-[var(--hi-accent)]' : 'group-focus-within:text-[#6f9978]'} ${iconMutedClass}`} />
                                            <input
                                                type="text"
                                                name="username"
                                    ...
                                    <div>
                                        ...
                                        <div className="group relative">
                                            <Mail className={`absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 transition ${false ? 'group-focus-within:text-[var(--hi-accent)]' : 'group-focus-within:text-[#6f9978]'} ${iconMutedClass}`} />
                                            <input
                                                type="email"
                                                name="email"
                                    ...
                                    <div>
                                        ...
                                        <div className="group relative">
                                            <Lock className={`absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 transition ${false ? 'group-focus-within:text-[var(--hi-accent)]' : 'group-focus-within:text-[#6f9978]'} ${iconMutedClass}`} />
                                            <input
                                    ...
                                    <label className={`flex items-start gap-3 rounded-[var(--hi-radius-md)] border px-4 py-3 text-sm ${panelMutedClass} ${panelSurfaceClass}`}>
                                        <input
                                            ...
                                            required
                                            className={`mt-0.5 h-4 w-4 rounded border ${isDark ? (false ? 'border-[var(--hi-border-strong)] bg-transparent' : 'border-white/16 bg-transparent') : 'border-[rgba(18,32,22,0.18)] bg-transparent'} ${false ? 'text-[var(--hi-accent)] focus:ring-[var(--hi-accent)]' : 'text-[#6f9978] focus:ring-[#6f9978]'} focus:ring-offset-0`}
                                        />
                                        <span>
                                            <Trans
                                                t={legalT}
                                                ...
                                                components={{
                                                    1: <Link to="/terms-of-service" target="_blank" className={`font-medium underline transition ${false ? 'text-[var(--hi-accent)] hover:text-[var(--hi-secondary-strong)]' : 'text-[#6f9978] hover:text-[#8bb395]'}`} />,
                                                    2: <Link to="/privacy-policy" target="_blank" className={`font-medium underline transition ${false ? 'text-[var(--hi-accent)] hover:text-[var(--hi-secondary-strong)]' : 'text-[#6f9978] hover:text-[#8bb395]'}`} />
                                                }}
                                            />
                                        </span>
                                    </label>
                                    ...
                                    <button
                                        type="submit"
                                        disabled={loading}
                                        className={`btn-primary !h-14 !w-full !rounded-[var(--hi-radius-md)] !px-5 text-base ${false ? '!shadow-[0_18px_36px_rgba(8,44,110,0.24)]' : '!shadow-[0_18px_36px_rgba(111,153,120,0.24)]'} disabled:opacity-60`}
                                    >
                                ...
                                <div className={`mt-8 border-t pt-5 ${dividerClass}`}>
                                    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                                        <p className={`text-sm ${panelMutedClass}`}>
                                            ...
                                            <Link to="/login" className={`font-semibold transition ${false ? 'text-[var(--hi-accent)] hover:text-[var(--hi-secondary-strong)]' : 'text-[#6f9978] hover:text-[#8bb395]'}`}>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
                    <div className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl dark:bg-slate-800">
                        ...
                        <div className="mb-6 space-y-4">
                            ...
                            <div className={`flex items-start gap-3 rounded-xl border p-4 ${false ? 'border-[rgba(103,227,242,0.24)] bg-[rgba(103,227,242,0.10)] dark:border-[rgba(103,227,242,0.28)] dark:bg-[rgba(103,227,242,0.12)]' : 'border-green-200 bg-green-50 dark:border-green-500/30 dark:bg-green-500/10'}`}>
                                <span className="text-2xl">2️⃣</span>
                                <p className={`text-sm ${false ? 'text-[var(--hi-accent)] dark:text-[var(--hi-accent)]' : 'text-green-700 dark:text-green-300'}`}>

...
function getStoredLanguagePreference(): string {
...
function getDomainDefaultLanguage(): string {
    if (typeof window === 'undefined') {
        return 'en';
    }
...
function applyRequestLanguage(lang: string): void {
    const normalized = normalizeLanguageCode(lang);
...
function installRequestLanguageInterceptor(): void {
...
i18n
    ...
    .init({
        ...
        fallbackLng: getFallbackLanguages as any,
        load: 'currentOnly',
        debug: false,
        interpolation: {
            escapeValue: false,
        ...
        detection: {
            ...
            convertDetectedLanguage: (lang: string) => lang
        },
        backend: {
            loadPath: (languages: string | string[]) => {

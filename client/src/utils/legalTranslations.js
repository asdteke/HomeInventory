...
const LEGAL_LONGFORM_VALIDATORS = {
    'legal.privacy_policy_content': (value) => {
    ...
    'legal.terms_of_service_content': (value) => {
        if (typeof value !== 'string') {
            return false;
        }
...
function getTranslationValue(i18n, language, key) {
...
function hasRequiredKeysForLanguage(i18n, language, requiredKeys) {
...
export function resolveCompleteTranslationLanguage(i18n, requiredKeys, fallbackLanguage = DEFAULT_LANGUAGE) {
...
export function resolveVerifiedLegalTranslationLanguage(i18n, requiredKeys, fallbackLanguage = DEFAULT_LANGUAGE) {
    const activeLanguage = normalizeLanguage(i18n?.resolvedLanguage || i18n?.language);

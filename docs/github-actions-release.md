...
HomeInventory v2.7.0 publishes two jointly versioned, independently verified artifact families:
...
2. Confirm root, client, launcher, Tauri, Rust, and lockfile versions are all `2.7.0` with `npm run version:check`.
3. Run CI and fix every build, test, audit, archive, launcher, and Rust failure.
4. Push the matching tag, or run `Launcher Packages` manually with `publish_release=true` and `release_tag=v2.7.0`. A tag push publishes automatically.
